...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        W_h = self.kernel_height;
        W_w = self.kernel_width;
        Cout = self.num_output_channels;
        Cin = self.num_input_channels;
        
        assert (self.input_height - W_h + 1 > 0)
        assert (self.input_width - W_w +  1 > 0)
        
        #calculate output dimensions
        N = inputs.shape[0];                              # batch size
        O_h = self.input_height - self.kernel_height + 1; # output height
        O_w = self.input_width - self.kernel_width + 1;   # output width
        outputs = np.zeros((N,Cout,O_h,O_w))
        for i in range(N):
            for j in range(Cout):
                outputs[i,j,:,:]+= self.biases[j];
                for k in range(Cin):
                    outputs[i,j,:,:] += scipy.signal.correlate2d(inputs[i,k,...],self.kernels[j,k,...],'valid');
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        Cin = self.num_input_channels;
        N = inputs.shape[0];
        gradsOutPad = np.pad(grads_wrt_outputs,((0,0),(0,0),(1,1),(1,1)),'constant')
        wrot = self.kernels[:,:,::-1,::-1]
        grads_wrt_inputs = np.zeros_like(inputs);
        for i in range(N):
            for j in range(Cin):
                for k in range(Cout):
                    grads_wrt_inputs[i,j,:,:] += scipy.signal.correlate2d(gradsOutPad[i,k,:,:],wrot[k,j,:,:],'valid');
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        #conv2d(inputs,grads_wrt_outputs):
        Cout = self.num_output_channels;
        Cin = self.num_input_channels;
        N = inputs.shape[0];
        ...
        for i in range(Cout):
            for j in range(Cin):
                for n in range(N):
                    grads_wrt_kernels[i,j,:,:] += scipy.signal.correlate2d(inputs[n,j,...],grads_wrt_outputs[n,i,...],'valid')
...
class MaxPooling2DLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        """
        assert (self.input_height % self.size) == 0;
        assert (self.input_width %self.size) == 0;
        out_height = (self.input_height // self.size);
        out_width  = (self.input_width // self.size);
        inputs_reshaped = inputs.reshape(inputs.shape[0],inputs.shape[1],out_height,self.size,out_width,self.size);
        outputs = inputs_reshaped.max(axis=3);
        outputs = outputs.max(axis=4);
        self.cache = inputs_reshaped;
        return outputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        """
        inputs_reshaped = self.cache;
        outputs_for_comparison = outputs[:,:,:,None,:,None];
        mask = np.array(inputs_reshaped == outputs_for_comparison,dtype=float);
        grads_wrt_outputs_broad = grads_wrt_outputs[:,:,:,None,:,None];
        grads_wrt_inputs = (mask) * grads_wrt_outputs_broad;
        return grads_wrt_inputs.reshape(inputs.shape);
